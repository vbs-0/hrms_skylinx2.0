...
from skylinx_audit.models import AuditModelConfig, AuditTag, SkylinxAuditInfo
...
class HistoryForm(forms.Form):
    ...
    def as_history_modal(self, *args, **kwargs):
        ...
        """
        context = {"form": self}
        table_html = render_to_string("skylinx_audit/skylinx_audit_log.html", context)
        return table_html
...
def _eligible_models():
    """Return concrete SkylinxModel subclasses that can be audit-tracked."""
    ...
    for model in django_apps.get_models():
        if not issubclass(model, SkylinxModel) or model._meta.abstract:
            continue
