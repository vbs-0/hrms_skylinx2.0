...
from skylinx_audit.methods import remove_duplicate_history
...
class SkylinxAuditInfo(models.Model):
    """
    SkylinxAuditInfo model to store additional info
    """
...
class SkylinxAuditLog(HistoricalRecords):
    """
    Model to store additional information for historical records.
    """

    # def __init__(self, *args, bases=None, **kwargs):
    #     super(SkylinxAuditLog, self).__init__(*args, **kwargs)
    #     self.is_skylinx_audit_log = True
...
def pre_create_skylinx_audit_log(sender, instance, *args, **kwargs):
    """
...
def post_create_skylinx_audit_log(sender, instance, *_args, **kwargs):
    """
    ...
    try:
        ...
        if isinstance(history_instance, SkylinxAuditLog):
