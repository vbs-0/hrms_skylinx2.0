...
from skylinx.signals import post_generic_delete, post_generic_import
from skylinx_auth.models import SkylinxUser
from skylinx_views.cbv_methods import hx_request_required, login_required
from skylinx_views.forms import DynamicBulkUpdateForm
...
class EmployeesList(SkylinxListView):
    """
    ...
    import_related_model_column_mapping = {
        "employee_user_id": base_models.SkylinxUser,
...
@method_decorator(login_required, name="dispatch")
class TabEmployeeWorkList(SkylinxListView):
    """
...
@method_decorator(login_required, name="dispatch")
class EmployeeWorkDetails(SkylinxDetailedView):
    """
...
class EmployeeNav(SkylinxNavView):
    """
    For nav bar
    """
...
@receiver(post_generic_import, sender=SkylinxUser)
    ...
    def _set_password(records):
        ...
        if users_to_update:
            ...
            with transaction.atomic():
                SkylinxUser.objects.bulk_update(users_to_update, ["password"])
                logger.info(
...
class EmployeeCard(SkylinxCardView):
    """
    For card view
    """
