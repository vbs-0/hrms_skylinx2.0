# Skylinx HRMS — Docker Deployment Guide

A complete, step-by-step guide to running Skylinx HR using Docker. Covers development setup, production deployment, customization, troubleshooting, and maintenance.
...
Get Skylinx running in under 5 minutes:

```bash
# 1. Clone the repository
git clone -b dev/v2.0 https://github.com/skylinx-opensource/skylinx.git
cd skylinx
...
| Variable | Default | Description |
|----------|---------|-------------|
| `POSTGRES_DB` | `skylinx_db` | Database name |
| `POSTGRES_USER` | `skylinx_user` | Database user |
| `POSTGRES_PASSWORD` | `skylinx_pass` | Database password — **change for production** |
...
docker compose exec web env GUNICORN_RELOAD=true gunicorn skylinx.wsgi:application --config docker/gunicorn.conf.py
...
# Redis CLI
docker compose exec redis redis-cli -a skylinx_pass
```
...
Change `skylinx_pass` in all locations:
...
From the Skylinx login page, click "Load Demo Data" to populate the system with sample employees, departments, and other test data.
...
Skylinx supports AWS S3 and Google Cloud Storage. Add to your environment:
...
DEFAULT_FILE_STORAGE=skylinx.skylinx_backends.PrivateMediaStorage
```
...
DEFAULT_FILE_STORAGE=skylinx.skylinx_backends_gcp.PrivateMediaStorage
```
...
```yaml
redis:
  command: redis-server --appendonly yes --requirepass skylinx_pass --maxmemory 256mb --maxmemory-policy allkeys-lru
```
...
```bash
# SQL backup
docker compose exec db pg_dump -U skylinx_user skylinx_db > backup_$(date +%Y%m%d_%H%M%S).sql

# Compressed backup
docker compose exec db pg_dump -U skylinx_user -F c skylinx_db > backup_$(date +%Y%m%d).dump
```
...
```bash
# From SQL file
cat backup.sql | docker compose exec -T db psql -U skylinx_user -d skylinx_db

# From compressed dump
docker compose exec -T db pg_restore -U skylinx_user -d skylinx_db --clean < backup.dump
```
...
```bash
# Backup media volume
docker run --rm -v skylinx_media:/data -v $(pwd):/backup alpine tar czf /backup/media_backup.tar.gz -C /data .

# Restore media volume
docker run --rm -v skylinx_media:/data -v $(pwd):/backup alpine tar xzf /backup/media_backup.tar.gz -C /data
```
...
2. Check database connections: `docker compose exec db psql -U skylinx_user -c "SELECT count(*) FROM pg_stat_activity;"`
3. Check Redis memory: `docker compose exec redis redis-cli -a skylinx_pass INFO memory`
...
- [ ] Database password is strong and unique (not `skylinx_pass`)
