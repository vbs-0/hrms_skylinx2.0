...
def ai_chat(request):
    ...
    HOWTO_GUIDES = (
        ...
        "contract. Emplinx auto-creates a DRAFT contract (wage 0) for every "
        "employee the first time their Work Info is saved — so a contract "
        "USUALLY ALREADY EXISTS. Go to sidebar Payroll > Pay Register, find "
        "the employee, OPEN/EDIT their existing draft (do not click Create — "
        "if a draft already exists Create will just show 'a draft contract "
        "already exists for this employee'), fill in real CTC/wage details, "
        "and change its status to Active. Only click Create if the employee "
        "genuinely has no contract row at all. Then sidebar Payroll > "
        "Payslips > Actions button (top right) > Generate. Pick employees + "
        "period > confirm. There is NO standalone 'Generate Payslip' button "
        "— it is inside the Actions dropdown.\n"
