...
@method_decorator(login_required, name="dispatch")
class AttendancesListView(SkylinxListView):
    """
    list view
    """
...
def _badge_count_from_attendance_list_view(request, view_cls):
    """
    Use the same queryset rules as each tab's SkylinxListView (filters, subordinates).
    """
    view = view_cls()
...
class AttendancesFormView(SkylinxFormView):
    """
    form view
    """
...
class AttendanceUpdateFormView(SkylinxFormView):
    """
    form for update
    """
...
@method_decorator(login_required, name="dispatch")
class PenaltyAccountListView(SkylinxListView):
    """
