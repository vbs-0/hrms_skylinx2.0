...
class AssetCategory(SkylinxModel):
    """
...
class Asset(SkylinxModel):
    ...
    objects = SkylinxCompanyManager("asset_category_id__company_id")
    ...
    def asset_status_col(self):
        ...
        if self.asset_status == "Available":
            ...
            label = f"{label}<span class='inline-block border-2 border-solid rounded font-bold text-[0.8rem] px-2 py-1 text-[hsl(258,84%,58%)] border-[hsl(258,84%,58%)] ms-5' title='{self.available_count} {self.get_asset_status_display()}'>{self.available_count}/{self.quantity}</span>"
            return label
...
class AssetReport(SkylinxModel):
    """
...
class AssetDocuments(SkylinxModel):
    """
...
class ReturnImages(SkylinxModel):
    """
...
class AssetRequest(SkylinxModel):
    """
