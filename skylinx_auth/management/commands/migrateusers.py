...
from skylinx_auth.models import SkylinxUser, LegacyUser


class Command(BaseCommand):
    help = "Migrate users from LegacyUser (auth_user) to SkylinxUser, including groups and permissions."
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for old_user in LegacyUser.objects.all():
                if SkylinxUser.objects.filter(username=old_user.username).exists():
                    skipped_count += 1
                    continue
                ...
                new_user = SkylinxUser.objects.create(
                    id=old_user.id,
