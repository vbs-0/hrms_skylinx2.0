...
def recruitment_delete_pipeline(request, rec_id):
    ...
    Returns:
        SkylinxRedirect: Used to refresh the page
    """
    try:
        recruitment_obj = Recruitment.objects.get(id=rec_id)
        recruitment_obj.delete()
...
def get_template(request, obj_id=None):
    ...
    if obj_id:
        body = (
            SkylinxMailTemplate.find(obj_id).body
            if SkylinxMailTemplate.find(obj_id)
            else None
        )
        if not body:
...
def get_template_hint(request, obj_id=None):
    ...
    if obj_id:
        body = SkylinxMailTemplate.objects.get(id=obj_id).body
