"""
Models for the skylinx_theme app
"""
...
class SkylinxColorTheme(SkylinxModel):
    ...
    class Meta:
        """
        Meta option for SkylinxColorTheme model
        """
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            if self.is_default:
                query = SkylinxColorTheme.objects.filter(is_default=True)
                if self.pk:
                    query = query.exclude(pk=self.pk)
                query.update(is_default=False)
            super().save(*args, **kwargs)
            if self.is_default and self.pk:
                SkylinxColorTheme.objects.filter(is_default=True).exclude(
                    pk=self.pk
...
class CompanyTheme(SkylinxModel):
    """
    ...
    def get_default_theme(cls):
        ...
        Returns the theme object or None
        """
        return SkylinxColorTheme.get_default_theme()
    ...
    def get_theme_for_company(cls, company):
        ...
        # return SkylinxColorTheme.objects.filter(
        #     name="Coral Red Theme (Default)"
        # ).first()
