# `skylinx_dbtemplate` — Database-backed Django templates
...
| **`skylinx.contrib.utils.middlewares`** | Loader reads `request` from `_thread_local`; ensure that middleware runs before template rendering where DB resolution is needed. |
...
- **`INSTALLED_APPS`**: include `django.contrib.sites` and `skylinx_dbtemplate` (or the app’s dotted path your project uses).
- **`SKYLINX_DBTEMPLATE_CACHE_BACKEND`**: optional; name of a key in `CACHES` (default: `"default"`). See `conf.py`.
- **`SKYLINX_DBTEMPLATE_AUTO_POPULATE_CONTENT`**: optional; if true, empty content can be filled from disk on save. See `conf.py`.
...
Place it according to product rules: usually **first among custom loaders** so active DB templates override packaged files. Exact `TEMPLATES` structure depends on your Skylinx project; search the repo for `Loader` or `skylinx_dbtemplate.loaders`.
...
4. **Cache** (`utils/cache.py`): keys are `skylinx_dbtemplate::<slugified_name>::<site_pk|global>::<slugified_lang>`. Invalidation must cover **all locale variants** in `LANGUAGES` plus `LANGUAGE_CODE`, because the loader uses `get_language()` while many rows use blank `language`.
...
Paths are relative to **`skylinx_dbtemplate/`**.

| Path | Role |
|------|------|
| **`__init__.py`** | Package docstring summarising resolution order and localhost / non-matching-site behaviour. |
| **`apps.py`** | `SkylinxDBTemplateConfig` (`AppLauncher`): app label, verbose name, **`auto_import_modules = ["signals"]`** so receivers load at startup. |
...
| **`conf.py`** | `SKYLINX_DBTEMPLATE_*` settings → cache backend name, `AUTO_POPULATE_CONTENT`, **`get_cache()`**. |
...
| **`templates/admin/skylinx_dbtemplate/template/*.html`** | Admin UI: **`restore_version.html`**, **`diff.html`**, **`preview.html`**. |
...
- **Key shape**: `skylinx_dbtemplate::{slugify(template_name)}::{site_id|global}::{slugify(language_or_all)}`.
