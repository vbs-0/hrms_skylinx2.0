...
from skylinx_meet.filters import GoogleMeetingFilter
from skylinx_meet.form import GoogleCloudCredentialForm, GoogleMeetingForm
from skylinx_meet.models import GoogleCloudCredential, GoogleCredential, GoogleMeeting
...
class GMeetCredentialSectionView(views.SkylinxSectionView):
    """
    View for displaying the Google Meet credentials section.

    Inherits from `SkylinxSectionView` to manage the section for Google Cloud credentials in the user interface.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(check_integration_enabled(app_name="skylinx_meet"), name="dispatch")
@method_decorator(
    permission_required("skylinx_meet.view_googlecloudcredential"), name="dispatch"
)
class GmeetCredentialNavView(views.SkylinxNavView):
    """
    View for the Google Meet credentials nav view.

    Inherits from `SkylinxNavView` to manage nav view for Google Cloud credentials. Handles dynamic attributes
...
class GmeetCredentialListView(views.SkylinxListView):
    """
    View for listing Google Meet credentials.

    Inherits from `SkylinxListView` to manage the listing of Google Cloud credentials with actions like edit and delete.
...
@method_decorator(check_integration_enabled(app_name="skylinx_meet"), name="dispatch")
@method_decorator(
    permission_required("skylinx_meet.add_googlecloudcredential"), name="dispatch"
)
class GoogleCredentialsFormView(views.SkylinxFormView):
    """
    View for creating or updating Google Cloud credentials.

    Inherits from `SkylinxFormView` to manage the form for creating or updating Google Cloud credentials.
    ...
    template_name = "generic/skylinx_form.html"
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("skylinx_meet.add_googlemeeting"), name="dispatch"
)
@method_decorator(check_integration_enabled(app_name="skylinx_meet"), name="dispatch")
class GmeetFormView(views.SkylinxFormView):
    """
    View for creating or updating Google Meet meetings.

    Inherits from `SkylinxFormView` to manage the form for creating or updating Google Meet meetings.
    Handles form validation, success, and failure responses. Displays success or error messages accordingly.
...
@method_decorator(check_integration_enabled(app_name="skylinx_meet"), name="dispatch")
@method_decorator(
    permission_required("skylinx_meet.view_googlemeeting"), name="dispatch"
)
class GmeetSectionView(views.SkylinxSectionView):
    """
    View for displaying the Google Meet section.

    Inherits from `SkylinxSectionView` to manage the section for Google Meet in the user interface.
...
class GmeetNavView(views.SkylinxNavView):
    """
    Nav view for Google Meet.

    Inherits from `SkylinxNavView` to manage nav bar for Google Meet view. Handles dynamic attributes for creating
...
class GmeetListView(views.SkylinxListView):
    """
    View for listing Google Meetings.

    Inherits from `SkylinxListView` to manage the listing of Google meetings with actions like edit, delete, and view meeting link.
...
class GmeetDetailedView(views.SkylinxDetailedView):
    """
    Detailed view for Google Meetings.

    Inherits from `SkylinxDetailedView` to manage the detailed view of a specific Google Meeting.
