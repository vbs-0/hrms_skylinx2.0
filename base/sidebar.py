...
Sections registered:
  ...
  - Theme Manager : color theme (only when skylinx_theme is installed)
...
def general_settings_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            ...
            "skylinx_audit.view_accountblockunblock",
...
def history_tags_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            ...
            "skylinx_audit.view_audittag",
        ]
    )


def audit_tracking_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("skylinx_audit.view_auditmodelconfig")
...
def color_theme_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("skylinx_theme.view_skylinxcolortheme")
...
def ldap_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("skylinx_ldap") and any(
        request.user.has_perm(p)
        for p in ["skylinx_ldap.add_ldapsettings", "skylinx_ldap.update_ldapsettings"]
    )


def google_meet_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("skylinx_meet") and request.user.has_perm(
        "skylinx_meet.view_googlecloudcredential"
    )
...
# ---------------------------------------------------------------------------
# 3. Theme Manager section (only when skylinx_theme is installed)
# ---------------------------------------------------------------------------
...
class ThemeManagerSettings:
    ...
    order = 10
    condition = lambda self, request: apps.is_installed("skylinx_theme")
    items = [
        {
            "label": _("Color Theme"),
            "url": reverse_lazy("skylinx_theme:color_theme_view"),
